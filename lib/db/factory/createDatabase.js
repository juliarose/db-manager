...
function createDatabase({credentialsPath, schemaPath, timezone}) {
    const getSettings = () => {
        return readFile(credentialsPath, 'utf8')
            .then(JSON.parse)
            .then((credentials) => {
                const stream = net.connect(credentials.socketPath).pipe(shape(10));
    ...
    const database = {
        ...
        query(queryStr) {
            ...
            return new Promise((resolve, reject) => {
                this.connection.query(queryStr, (error, rows) => {
                    if (error) {
                        reject(error);
                    } else {
                        resolve(rows);
        ...
        setup() {
            return getSettings()
                ...
                .then((settings) => {
                    ...
                    database.connection = mysql.createConnection(settings);
